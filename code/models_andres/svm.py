...
print(classification_report(y_val_cls, val_cls_preds))

print("=" * 50)
print("SVM — Classification (Test)")
print("=" * 50)
test_cls_preds = cls_model.predict(X_test)
print(classification_report(y_test_cls, test_cls_preds))
...
val_report  = classification_report(y_val_cls,  val_cls_preds,  output_dict=True)
test_report = classification_report(y_test_cls, test_cls_preds, output_dict=True)

fig, axes = plt.subplots(1, 3, figsize=(16, 5))
fig.suptitle("SVM — Classification Results", fontsize=15, fontweight="bold")
...
ax = axes[2]
cm = confusion_matrix(y_test_cls, test_cls_preds, labels=CLASSES)
...
plt.show()
print("Saved → svm_classification.png")

# REGRESSION PLOTS
fig, axes = plt.subplots(1, 3, figsize=(16, 5))
fig.suptitle("SVR — Regression Results", fontsize=15, fontweight="bold")
...
plt.tight_layout()
plt.savefig("svm_regression.png", dpi=150)
plt.show()
print("Saved: svm_regression.png")