...
cls_base_estimators = [
    ...
    ("svm", SVC(kernel="rbf", C=1.0, gamma="scale", class_weight="balanced",
                probability=True, random_state=42)),   # probability=True required for stacking
    ("knn", KNeighborsClassifier(n_neighbors=5)),
    ("mlp", MLPClassifier(hidden_layer_sizes=(64, 32), max_iter=300,
                          early_stopping=False, random_state=42)),  # early_stopping off — string label compatibility
]
...
# cv=5: base models are trained on 5 folds to generate out-of-fold predictions
print("Training Stacking Classifier (this may take a minute)...")

stacking_cls = StackingClassifier(
    estimators=cls_base_estimators,
...
stacking_cls.fit(X_train, y_train_cls)

print("\n" + "=" * 55)
print("STACKING ENSEMBLE — Classification (Val)")
print("=" * 55)
val_preds = stacking_cls.predict(X_val)
print(classification_report(y_val_cls, val_preds))

print("=" * 55)
print("STACKING ENSEMBLE — Classification (Test)")
print("=" * 55)
test_preds = stacking_cls.predict(X_test)
print(classification_report(y_test_cls, test_preds))

# STACKING — REGRESSION
# Meta-learner: Ridge Regression
print("\nTraining Stacking Regressor (this may take a minute)...")

stacking_reg = StackingRegressor(
    estimators=reg_base_estimators,
...
print("\n" + "=" * 55)
print("STACKING ENSEMBLE — Regression (Val)")
print("=" * 55)
val_reg_preds = stacking_reg.predict(X_val)
...
print("\n" + "=" * 55)
print("STACKING ENSEMBLE — Regression (Test)")
print("=" * 55)
test_reg_preds = stacking_reg.predict(X_test)
rmse = np.sqrt(mean_squared_error(y_test_reg, test_reg_preds))
mae  = mean_absolute_error(y_test_reg, test_reg_preds)
r2   = r2_score(y_test_reg, test_reg_preds)
...
fig.suptitle("Stacking Ensemble — Classification Results", fontsize=15, fontweight="bold")
...
# REGRESSION PLOTS
fig, axes = plt.subplots(1, 3, figsize=(16, 5))
fig.suptitle("Stacking Ensemble — Regression Results", fontsize=15, fontweight="bold")
...
plt.tight_layout()
plt.savefig("ensemble_regression_scatter.png", dpi=150)
plt.show()
print("Saved: ensemble_regression_scatter.png")